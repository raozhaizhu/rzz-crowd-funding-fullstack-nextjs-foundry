...
import SpringMotionText from "@/components/spring-motion-text";
...
const ContractPageComponent = ({ slug }: { slug: string }) => {
  ...
  useEffect(() => {
    if (donationInfoError) toast.error(donationInfoError.message);
  }, [donationInfoError]);
  ...
  if (isPending || donationInfoPending) {
    // 数据还没加载，给个 loading UI
    return <SpringMotionText text='Loading...'></SpringMotionText>;
  }

  if (!campaign) {
    // 数据加载完了，但没找到对应数据，才跳404
    return notFound();
  }
  // ANCHOR Render
  return (
    <section className='container md:pt-25 mx-auto flex flex-col items-center gap-4 md:gap-8 lg:gap-12'>
      {/* title & description */}
